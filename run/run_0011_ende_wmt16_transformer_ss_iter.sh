...
# ------------------------ EXP --------------------------
ss_max=0.3
exp_name=0011_transformer_ss${ss_max}_iter
# exp_name=asup
...
fairseq-train \
    ...
    --criterion label_smoothed_cross_entropy_ss --ss-max ${ss_max} --label-smoothing 0.1 \
    --max-tokens 3584 \
    --fp16 --update-freq 8 \
    --save-dir $save_dir \
...
fairseq-generate \
    ...
    --beam 4 --lenpen 0.6 --remove-bpe --results-path $save_dir
    # > $save_dir/gen_asup.out
;;
