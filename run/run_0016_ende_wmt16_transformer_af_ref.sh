...
scale_attn_loss=500.0
exp_name=0016_transformer_af_ref_scale${scale_attn_loss}
...
fairseq-train \
    ...
    --criterion label_smoothed_cross_entropy_af_ref --model-tf-path ${EXP_DIR}${tf_model} --scale-attn-loss $scale_attn_loss --label-smoothing 0.1 \
    --max-tokens 3584 \
    --fp16 --update-freq 8 \
    --save-dir $save_dir \
