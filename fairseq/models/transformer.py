...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def update_params(self, idx_layer_lst = [4,5]):
        ...
        for idx in idx_layer_lst:
            n_lst += [ n for n, p in self.named_parameters() if f'decoder.layers.{idx}.encoder_attn' in n]
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        # print(layer_attn.shape, attn.shape, x.shape)
...
# ------------- af -------------
@register_model("transformer_af")
class TransformerModel_af(TransformerModel):
    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TransformerDecoder_af(
            args,
            tgt_dict,
            embed_tokens,
            no_encoder_attn=getattr(args, "no_cross_attention", False),
    ...
    def forward(
        ...
        decoder_out = self.decoder(
            ...
            src_lengths=src_lengths,
            return_all_hiddens=return_all_hiddens,
            attn_all_ref=attn_all_ref,
        )
        return decoder_out


class TransformerDecoder_af(TransformerDecoder):
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = TransformerDecoderLayer_af(args, no_encoder_attn)
        if getattr(args, "checkpoint_activations", False):
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
        return layer
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _ = layer(
                ...
                self_attn_mask=self_attn_mask,
                self_attn_padding_mask=self_attn_padding_mask,
                need_attn=True, # bool((idx == alignment_layer))
                need_head_weights=True, # bool((idx == alignment_layer))
                attn_ref=attn_all_ref[idx] if attn_all_ref is not None else None,
            )
            inner_states.append(x)
            if layer_attn is not None: # and idx == alignment_layer:
                attn = layer_attn.float().to(x)
                if alignment_heads is not None: attn = attn[:alignment_heads]
...
@register_model_architecture("transformer_af", "transformer_vaswani_wmt_en_de_big_af")
def transformer_vaswani_wmt_en_de_big_af(args):
