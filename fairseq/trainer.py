...
class Trainer(object):
    ...
    def update_params(self, idx_layer_lst=[4,5]):
        """
        obsolete, now written in transformer.py
        """
        # import pdb; pdb.set_trace()
        # n_lst = [ n for n, p in self.model.named_parameters() ]
        # p_lst = [ p for n, p in self.model.named_parameters() ]
        # print(len(n_lst), len(p_lst))
        # print(n_lst[0], p_lst[0])

        n_lst = []
        for idx in idx_layer_lst:
            n_lst += [ n for n, p in self.model.named_parameters() if f'decoder.layers.{idx}.encoder_attn' in n]
        for n, p in self.model.named_parameters():
            p.requires_grad = True if n in n_lst else False

        # p_lst = [ p for n, p in self.model.named_parameters() if n in n_lst ]
        # tmp = [ p for n, p in self.model.named_parameters() if not n in n_lst ]
        # print(len(tmp), len(p_lst))
        # print(p_lst[0].requires_grad, tmp[0].requires_grad)
        # import pdb; pdb.set_trace()


class Trainer_af(Trainer):
    """
    trainer for attention forcing
    """

    def __init__(self, cfg: FairseqConfig, task, model, model_tf, criterion, quantizer=None):
        # super().__init__(cfg, task, model, criterion, quantizer=quantizer)
        ...
        self._criterion = criterion
        self._model = model
        self._model_tf = model_tf
        if cfg.common.fp16:
            self._criterion = self._criterion.half()
            self._model = self._model.half()
            self._model_tf = self._model_tf.half()
        elif cfg.common.bf16:
            self._criterion = self._criterion.to(dtype=torch.bfloat16)
            self._model = self._model.to(dtype=torch.bfloat16)
            self._model_tf = self._model_tf.to(dtype=torch.bfloat16)
        if (
            not cfg.distributed_training.pipeline_model_parallel
            # the DistributedFairseqModel wrapper will handle moving to device,
            # so only handle cases which don't use the wrapper
            ...
            self._criterion = self._criterion.to(device=self.device)
            self._model = self._model.to(device=self.device)
            self._model_tf = self._model_tf.to(device=self.device)
        self.pipeline_model_parallel = cfg.distributed_training.pipeline_model_parallel
        self.last_device = None
        if self.cuda and self.pipeline_model_parallel:
            self.last_device = torch.device(
        ...
        for shared_param in _catalog_shared_params(model_tf):
            ref = _get_module_by_path(self._model_tf, shared_param[0])
            for path in shared_param[1:]:
                logger.info(
                    "detected shared parameter: {} <- {}".format(shared_param[0], path)
                )
                _set_module_by_path(self._model_tf, path, ref)
    ...
    @property
    def model_tf(self):
        if self._wrapped_model_tf is None:
            if self.use_distributed_wrapper:
                self._wrapped_model_tf = models.DistributedFairseqModel(
                    self.cfg.distributed_training,
                    self._model_tf,
                    process_group=self.data_parallel_process_group,
                    device=self.device,
                ...
                self._wrapped_model_tf = self._model_tf
        return self._wrapped_model_tf
    ...
    def load_checkpoint(
        ...
        filename_tf = self.criterion.model_tf_path
        logger.info(f"Preparing to load checkpoint tf {filename_tf}")
        ...
        if bexists:
            ...
            if load_on_all_ranks or self.data_parallel_rank == 0:
                ...
                state_tf = checkpoint_utils.load_checkpoint_to_cpu(
                    filename_tf, load_on_all_ranks=load_on_all_ranks
                )
                last_optim_state = state.get("last_optimizer_state", None)
            ...
            if is_distributed and not load_on_all_ranks:
                ...
                state_tf = distributed_utils.broadcast_object(
                    state_tf,
                    src_rank=0,
                    group=self.data_parallel_process_group,
                    dist_device=self.device,
            ...
            # import pdb; pdb.set_trace()
            # print(state.keys())
            # print(utils.has_parameters(self.get_criterion()))
            # tmp = self.get_criterion()
            # print(type(tmp))
            # print(tmp.parameters())
            # print(tmp)
            # print(next(tmp.parameters()))
            try:
                self.model.load_state_dict(
                    state["model"], strict=True, model_cfg=self.cfg.model
                )
                self.model_tf.load_state_dict(
                    state_tf["model"], strict=True, model_cfg=self.cfg.model
                )
                if utils.has_parameters(self.get_criterion()):
                    self.get_criterion().load_state_dict(
                        state["criterion"], strict=True
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.model_tf, self.criterion
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
